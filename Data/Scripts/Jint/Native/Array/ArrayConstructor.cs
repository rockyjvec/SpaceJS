...
namespace Jint.Native.Array
{
    public sealed class ArrayConstructor : FunctionInstance, IConstructor
    {
        ...
        private JsValue From(JsValue thisObj, JsValue[] arguments)
        {
            ...
            if (source is IArrayLike)
            {
                var arrayLike = source as IArrayLike;
        ...
        public ArrayInstance Construct(JsValue[] arguments, uint capacity)
        {
            ...
            else if (arguments.Length == 1 && arguments[0] is ObjectWrapper)
            {
                var objectWrapper = arguments[0] as ObjectWrapper;
                if (objectWrapper.Target is IEnumerable)
                {
                    var enumerable = objectWrapper.Target as IEnumerable;
