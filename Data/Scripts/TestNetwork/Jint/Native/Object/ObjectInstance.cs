...
namespace Jint.Native.Object
{
    public class ObjectInstance : JsValue, IEquatable<ObjectInstance>
    {
        ...
        {
            if (this is IObjectWrapper)
            {
                var wrapper = this as IObjectWrapper;
            ...
            {
                case "Array":
                    if (this is ArrayInstance)
                    {
                        var arrayInstance = this as ArrayInstance;
                ...
                case "String":
                    ...
                    {
                        var stringInstance = this as StringInstance;
...
*/
                case "Boolean":
                    ...
                    {
                        var booleanInstance = this as BooleanInstance;
                ...
                case "Function":
                    if (this is FunctionInstance)
                    {
                        var function = this as FunctionInstance;
                ...
                case "Number":
                    if (this is NumberInstance)
                    {
                        var numberInstance = this as NumberInstance;
                ...
                case "RegExp":
                    if (this is RegExpInstance)
                    {
                        var regeExpInstance = this as RegExpInstance;
            ...
            out JsValue value)
        {
            ...
            {
                GetLength = TypeConverter.ToUint32(descValue);
            }
            else
            {                
                var getter = desc.Get ?? Undefined;
                ...
                {
                    // if getter is not undefined it must be ICallable
                    GetLength = TypeConverter.ToUint32(((ICallable) getter).Call(this, Arguments.Empty));
                }
            }
            ...
            {
                JsValue kvalue;
                var property = TypeConverter.ToString(k);
                var kPresent = HasProperty(property);
                kvalue = kPresent ? Get(property) : Undefined;
                if (kPresent)
                {
                    args[0] = kvalue;
                    args[1] = k;
        ...
        {
            if (source is ICallable)
            {
                var callable = source as ICallable;
                return callable;
            }
